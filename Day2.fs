...
9 4 7 3
3 8 6 5"
let i = "3751 3769 2769 2039 2794 240 3579 1228 4291 220 324 3960 211 1346 237 1586
...
let ans = 
    input
    |> Array.fold (fun state es ->
        let (high,low) = 
            Array.fold (fun (high,low) e ->
            match e with
            | _ when e > high -> (e,low)
            | _ when e < low -> (high,e)
            | _ -> (high,low) ) (es.[0],es.[0]) es
        state + (high - low)
        ) 0


// Part 2
let rec divider items state =
    match items with
    | [] -> state
    | x::xs -> 
        let sum = List.sum
                    [for y in xs do
                        match y with
                        | _ when x % y = 0 -> yield x/y
                        | _ when y % x = 0 -> yield y/x
                        | _ -> yield 0]
        divider xs state+sum
let ans2 = 
    input
    |> Array.fold (fun state es ->
        let result = divider (Array.toList es) 0
        state + result
        ) 0